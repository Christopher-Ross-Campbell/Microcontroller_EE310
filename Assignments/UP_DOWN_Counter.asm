...
;  	V1.2: 03/27/2024 - Added Additional Comments
;-----------------------------
...
;----------------
	...
	REG20 EQU 0x20   ;Set up registers for delay and counter
	REG21 EQU 0x21
	REG22 EQU 0x22
...
_initialization: 
	RCALL _setupPortA	;Initialize registers
	RCALL _setupPortD
...
RESET_0:			;Reset after initialization and both buttons pressed
	LFSR  0,0x00
	MOVFF INDF0,PORTD
	MOVLW 0x00
	MOVWF CNTR
...
MAIN_LOOP:			;Main loop checking for botton positions
	MOVLW 0x00
	MOVWF STATUS
	...
	GOTO  MAIN_LOOP
	NOP
COUNT_UP:			;Count up, runs once per button check
...
COUNT_DOWN:			;Count down, runs once per button check
	MOVLW 0x00
	MOVWF STATUS
	DECF CNTR,1
	BTFSC STATUS,4
...
DEC_RESET:		;Call when FSR reach max increment travel
	LFSR 0,0x10
	MOVLW 0x0F
	MOVWF CNTR
	RETURN
INC_RESET:		;Call when FSR reach max decrement travel
	LFSR 0,0x00
	MOVLW 0x00
	MOVWF CNTR
	RETURN
